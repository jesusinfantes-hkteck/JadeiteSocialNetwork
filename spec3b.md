# ARIADNA TASK SPEC
# Version: 1.0.0
# Generated: 2026-05-21T15:12:53.2332228+00:00
...
### AC-01: Account unlocks after expiry

- **Dado** a user account locked after 5 failed login attempts in Login.aspx.cs
- **Cuando** 15 minutes have elapsed since the lockout occurred
- **Entonces** the user can successfully log in with correct credentials without admin intervention
- **Verificar con**: Trigger 5 failed logins, wait 15 minutes, then verify LogIn method accepts valid credentials and sets Session['AccountLocked_' + email] to null or false

### AC-02: Lockout persists before expiry

- **Dado** a user account locked after 5 failed login attempts
- **Cuando** the user attempts to log in with correct credentials before 15 minutes have passed
- **Entonces** the IsLockedOut method returns true and FailureText.Text displays 'Account locked.'
- **Verificar con**: Trigger 5 failed logins, wait 5 minutes, attempt login and verify ErrorMessage.Visible is true with lockout message

### AC-03: Failed attempts reset after unlock

- **Dado** an account that was previously locked and automatically unlocked after 15 minutes
- **Cuando** the user successfully logs in after the lockout expiry
- **Entonces** the Session['FailedAttempts_' + email] counter is reset to zero or null
- **Verificar con**: After 15-minute auto-unlock and successful login, verify ResetFailedAttempts method clears both FailedAttempts and AccountLocked session keys
...
{
  "specVersion":      "1.0.0",
  "generatedAt":      "2026-05-21T15:12:53.2332228+00:00",
